...
[role="lead"]
Crie uma relação de replicação para um sistema de arquivos FSx for ONTAP no NetApp Workload Factory para evitar perda de dados em caso de desastre imprevisto. A replicação é compatível entre dois sistemas de arquivos FSx for ONTAP e entre Cloud Volumes ONTAP ou um sistema ONTAP local e um sistema de arquivos FSx for ONTAP.

.Sobre esta tarefa
A replicação adiciona proteção contra perda de dados se a região onde seus dados residem sofrer um desastre.
