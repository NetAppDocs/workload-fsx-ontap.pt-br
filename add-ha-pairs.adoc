...
.Resultado
O Workload Factory inicia o processo para adicionar o(s) par(es) de HA ao sistema de arquivos. O sistema de arquivos não está disponível para outras operações como criação de volumes até que os pares de HA sejam adicionados. Você pode monitorar o progresso da operação na guia *Tracker*.
