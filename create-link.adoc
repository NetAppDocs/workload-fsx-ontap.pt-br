...
NOTE: O AWS Secrets Manager não é compatível ao usar um agente do Console.
...
** Para análise de eventos do CloudShell e do FSx para o Sistema de Gerenciamento de Emergências (EMS) do ONTAP : porta 22 (SSH)


* O link deve ser capaz de se conectar ao seguinte endpoint: https://api.workloads.netapp.com. O console baseado na web contata este endpoint para interagir com as APIs do Workload Factory, a fim de gerenciar e operar cargas de trabalho do FSx para ONTAP .
...
+
Observe que três permissões são concedidas ao Lambda quando você inicia o modelo CloudFormation. O NetApp Workload Factory usa essas permissões ao utilizar links.
