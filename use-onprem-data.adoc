...
summary: Descubra e replique os dados ONTAP locais no NetApp Workload Factory para que possam ser usados como backup na nuvem. 
---
...
[role="lead"]
Descubra e replique os dados ONTAP locais no NetApp Workload Factory para que possam ser usados como backup na nuvem.
...
* Um link conectado para associar ao cluster local descoberto. Se você não tiver um link, será necessário link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["crie um"^].
...
.. *Credenciais ONTAP*: Forneça o seguinte:
+
*** GovCloud contas: insira o ARN com as credenciais do ONTAP.
*** Todas as outras contas: insira as credenciais do ONTAP para o cluster ONTAP local. Certifique-se de que o usuário tenha as permissões necessárias.
...
* Tanto o cluster ONTAP local quanto o sistema de arquivos FSx for ONTAP que você usa para a relação de replicação devem ter um link associado. link:https://docs.netapp.com/us-en/workload-setup-admin/create-link.html["Aprenda como associar um link existente ou criar e associar um novo link"^]. Após a associação do link, retorne a esta operação.
