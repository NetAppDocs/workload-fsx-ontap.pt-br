...
summary: Depois de configurar seu sistema de arquivos FSX for ONTAP, você pode criar o FSX for ONTAP volumes. 
---
...
[role="lead"]
Depois de configurar seu sistema de arquivos FSX for ONTAP, você pode criar o FSX for ONTAP volumes.
...
* Você deve ter um sistema de arquivos FSX for ONTAP no Workload Factory.
* Você precisa ter uma VM de storage.
* Para acesso ao protocolo, execute o seguinte:
+
** Para configurar o acesso ao volume, é link:manage-links.html["associe um link"]necessário . Se você não tiver um link existente, link:create-link.html["crie um link"]. Para associar um link no sistema de arquivos, clique em *associar link* em *nome da conta*. Assim que o link se associar, volte a esta operação.
...
. Na guia *Visão geral*, clique em *criar volume*.
...
.... *Compartilhamento CIFS*: Insira o nome do compartilhamento CIFS para fornecer acesso SMB.


*** *Acesso por bloco*: Permite aos hosts que executam aplicativos empresariais críticos acesso ao volume usando o protocolo iSCSI.
+
